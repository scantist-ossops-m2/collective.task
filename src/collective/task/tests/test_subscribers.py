...
class TestSubscribers(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.task2 = api.content.create(
            ...
            enquirer="Administrators",
        )
        self.task3 = api.content.create(
            container=self.task2,
            type="task",
            id="task3",
            title="Task3",
            assigned_group="Reviewers",
            enquirer="Site Administrators",
        )
        self.task4 = api.content.create(container=self.task3, type="task", id="task4", title="Task4", assigned_group="")
    ...
    def test_taskContent_created(self):
        ...
        self.assertListEqual(
            self.task1["task2"]["task3"].parents_assigned_groups, ["Site Administrators", "Administrators"]
        )
        self.assertListEqual(
            self.task1["task2"]["task3"]["task4"].parents_assigned_groups,
            ["Site Administrators", "Administrators", "Reviewers"],
        )
        self.assertEqual(self.task1.parents_enquirers, None)
        self.assertEqual(self.task1["task2"].parents_enquirers, ["Reviewers"])
        self.assertListEqual(self.task1["task2"]["task3"].parents_enquirers, ["Reviewers", "Administrators"])
        self.assertListEqual(
            self.task1["task2"]["task3"]["task4"].parents_enquirers,
        ...
        self.assertListEqual(self.task4.parents_assigned_groups, ["Site Administrators", "Administrators", "Reviewers"])
        self.assertEqual(self.task1.parents_enquirers, None)
        ...
        self.assertListEqual(self.task4.parents_enquirers, ["Reviewers", "Administrators", "Site Administrators"])
